...
class DataService:
    ...
    @retry(max_retries=3)
    def get_all_stocks_daily(self, target_date: str) -> Optional[pd.DataFrame]:
        """
        批量获取指定日期全市场股票的日行情数据（带缓存）。
        比逐只调用 get_daily_kline 快几个数量级。

        参数:
            target_date: 日期字符串 YYYY-MM-DD

        返回 DataFrame: [code, close, change_pct, volume, turnover]
        """
        cache_key = f"all_daily_{target_date}"
        cached = self._load_cache(cache_key)
        if cached is not None:
            return cached

        logger.info(f"正在批量获取 {target_date} 全市场日行情...")
        try:
            _ensure_bs_login()
            ...
            if not all_rows:
                logger.warning(f"{target_date} 无行情数据")
                return None

            df = pd.DataFrame(all_rows)
            for col in ["close", "volume", "turnover", "change_pct"]:
                df[col] = pd.to_numeric(df[col], errors="coerce")

            # 过滤停牌股（volume=0 或 NaN）
            df = df[df["volume"] > 0].reset_index(drop=True)

            self._save_cache(cache_key, df)
            logger.info(f"{target_date} 行情数据获取完成，共{len(df)}条")
            return df

        except Exception as e:
            logger.error(f"批量获取{target_date}行情失败: {e}")
            return None
